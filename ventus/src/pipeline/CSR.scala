...
//               Mux(AVL>(VLMAX<<1.U).asUInt,VLMAX,AVL>>1.U))
  ...
  when(io.CTA2csr.valid){
    ...
    wf_tag_dispatch := (if (num_warp == 1) 0.U else io.CTA2csr.bits.CTAdata.dispatch2cu_wf_tag_dispatch(depth_warp-1,0))// wf_tag_dispatch :=io.CTA2csr.bits.CTAdata.dispatch2cu_wf_tag_dispatch
    ...
    threadid := (if (num_warp == 1) 0.U else io.CTA2csr.bits.CTAdata.dispatch2cu_wf_tag_dispatch(depth_warp-1,0)) << depth_thread//threadid:=io.CTA2csr.bits.CTAdata.dispatch2cu_wf_tag_dispatch(depth_thread-1,0)<<depth_thread
  }
