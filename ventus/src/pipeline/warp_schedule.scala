...
class warp_scheduler extends Module{
  ...
  val new_wg_id = if (WF_ID_WIDTH == 0) io.warpReq.bits.CTAdata.dispatch2cu_wf_tag_dispatch else io.warpReq.bits.CTAdata.dispatch2cu_wf_tag_dispatch(TAG_WIDTH-1, WF_ID_WIDTH)
  val new_wf_id = if (WF_ID_WIDTH == 0) 0.U(1.W) else io.warpReq.bits.CTAdata.dispatch2cu_wf_tag_dispatch(WF_ID_WIDTH-1,0)
  val new_wg_wf_count=io.warpReq.bits.CTAdata.dispatch2cu_wg_wf_count
  val end_wg_id = if (WF_ID_WIDTH == 0) io.wg_id_tag else io.wg_id_tag(TAG_WIDTH-1, WF_ID_WIDTH)
  val end_wf_id = if (WF_ID_WIDTH == 0) 0.U(1.W) else io.wg_id_tag(WF_ID_WIDTH-1, 0)
