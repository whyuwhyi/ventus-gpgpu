...
object Instructions {
  ...
  def CSRRSV             = BitPat("b?????????????????010?????1110010")
  ...
  // custom MMA instructions, opcode = 0x0A
  def MMA_WARP_BASE        = BitPat("b?????????????????????????0001010")
  def MMA_M8N8K16          = BitPat("b????????000??????????????0001010")
  def MMA_M16N8K16         = BitPat("b????????001??????????????0001010")
  def MMA_M8N16K16         = BitPat("b????????010??????????????0001010")
  def MMA_M16N16K16        = BitPat("b????????011??????????????0001010")
  def MMA_M8N8K8           = BitPat("b????????100??????????????0001010")
  def MMA_M16N8K8          = BitPat("b????????101??????????????0001010")
  def MMA_M8N16K8          = BitPat("b????????110??????????????0001010")
  def MMA_M16N16K8         = BitPat("b????????111??????????????0001010")

  // custom shuffle instructions, opcode = 0x42
  def SHUFFLE_IDX          = BitPat("b0010011??????????001?????1000010")
  def SHUFFLE_UP           = BitPat("b0010101??????????001?????1000010")
  def SHUFFLE_DOWN         = BitPat("b0010111??????????001?????1000010")
  def SHUFFLE_BFLY         = BitPat("b0010001??????????001?????1000010")

  // custom packed FPU instructions, opcode = 0x5A
  def VADD_F16X2              = BitPat("b0000001??????????000?????1011010")
  def VMUL_F16X2              = BitPat("b0000011??????????000?????1011010")
  def VFMA_F16X2              = BitPat("b0000101??????????000?????1011010")
  def VADD_BF16X2             = BitPat("b0000001??????????001?????1011010")
  def VMUL_BF16X2             = BitPat("b0000011??????????001?????1011010")
  def VFMA_BF16X2             = BitPat("b0000101??????????001?????1011010")

  // custom vector convert instructions, opcode = 0x7A
  def VCVT_FP32_FP16          = BitPat("b0000001?????00000000?????1111010")
  def VCVT_FP16_FP32          = BitPat("b0000011?????00000000?????1111010")
  def VCVT_FP32_BF16          = BitPat("b0000101?????00000000?????1111010")
  def VCVT_BF16_FP32          = BitPat("b0000111?????00000000?????1111010")

  // custom UNFU instructions, opcode = 0x2A
  def VEX2_APPROX_F32        = BitPat("b0000001?????00000000?????0101010")
  def VLG2_APPROX_F32        = BitPat("b0000011?????00000000?????0101010")
  def VRCP_APPROX_F32        = BitPat("b0000101?????00000000?????0101010")
  def VSQRT_APPROX_F32       = BitPat("b0000111?????00000000?????0101010")
  def VRSQRT_APPROX_F32      = BitPat("b0001001?????00000000?????0101010")
  def VSIN_APPROX_F32        = BitPat("b0001011?????00000000?????0101010")
  def VCOS_APPROX_F32        = BitPat("b0001101?????00000000?????0101010")
  def VTANH_APPROX_F32       = BitPat("b0001111?????00000000?????0101010")
  def VGELU_APPROX_F32       = BitPat("b0010001?????00000000?????0101010")
  def VSILU_APPROX_F32       = BitPat("b0010011?????00000000?????0101010")

  def VEX2_APPROX_F16X2      = BitPat("b0000001?????00000001?????0101010")
  def VRCP_APPROX_F16X2      = BitPat("b0000101?????00000001?????0101010")
  def VSQRT_APPROX_F16X2     = BitPat("b0000111?????00000001?????0101010")
  def VRSQRT_APPROX_F16X2    = BitPat("b0001001?????00000001?????0101010")
  def VTANH_APPROX_F16X2     = BitPat("b0001111?????00000001?????0101010")
  def VGELU_APPROX_F16X2     = BitPat("b0010001?????00000001?????0101010")
  def VSILU_APPROX_F16X2     = BitPat("b0010011?????00000001?????0101010")

  def VEX2_APPROX_BF16X2     = BitPat("b0000001?????00000010?????0101010")
  def VRCP_APPROX_BF16X2     = BitPat("b0000101?????00000010?????0101010")
  def VSQRT_APPROX_BF16X2    = BitPat("b0000111?????00000010?????0101010")
  def VRSQRT_APPROX_BF16X2   = BitPat("b0001001?????00000010?????0101010")
  def VTANH_APPROX_BF16X2    = BitPat("b0001111?????00000010?????0101010")
  def VGELU_APPROX_BF16X2    = BitPat("b0010001?????00000010?????0101010")
  def VSILU_APPROX_BF16X2    = BitPat("b0010011?????00000010?????0101010")
